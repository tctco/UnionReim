...
export function registerIpcHandlers(): void {
    ...
    const tryAutoWatermarkAttachments = async (attachments: Attachment[]) => {
        ...
        for (const a of attachments) {
            ...
            try {
                await watermarkService.applyWatermark(a.attachment_id);
            } catch {
                /* ignore */
            }
        }
    ...
    ipcMain.handle(
        "settings:update",
        respond((request: SettingsUpdateRequest) => {
    ...
    ipcMain.handle(
        "settings:getSetting",
        respond((key: string) => settingsService.getSetting(key)),
    );
    ...
    ipcMain.handle(
        "settings:signatureUploadFromPath",
        respond((req: { path: string; original_name?: string }) => {
    ...
    ipcMain.handle(
        "settings:signatureUploadFromData",
        respond((req: { data: number[]; name?: string; mime?: string }) => {
    ...
    ipcMain.handle(
        "template:create",
        respond((request: CreateTemplateRequest) => templateService.createTemplate(request)),
    );

    ipcMain.handle(
        "template:list",
        respond((filter?: { search?: string }) => templateService.listTemplates(filter)),
    );

    ipcMain.handle(
        "template:get",
        respond((template_id: number) => {
    ...
    ipcMain.handle(
        "template:update",
        respond((request: UpdateTemplateRequest) => {
    ...
    ipcMain.handle(
        "template:delete",
        respond((template_id: number) => templateService.deleteTemplate(template_id), { successFromBoolean: true }),
    );

    ipcMain.handle(
        "template:safeDelete",
        respond((template_id: number) => templateService.safeDeleteTemplate(template_id)),
    );

    ipcMain.handle(
        "template:checkModification",
        respond((template_id: number, critical_changes?: boolean) =>
            templateService.canModifyTemplateItem(template_id, critical_changes),
        ),
    );

    ipcMain.handle(
        "template:getAssociatedProjects",
        respond((template_id: number) => templateService.getAssociatedProjects(template_id)),
    );

    ipcMain.handle(
        "template:export",
        respond(async (request: TemplateExportRequest) => {
    ...
    ipcMain.handle(
        "system:isDirectoryEmpty",
        respond((dirPath: string) => {
            try {
                if (!dirPath || typeof dirPath !== "string") return false;
                if (!existsSync(dirPath)) return true; // treat non-existent as empty/ok
    ...
    ipcMain.handle(
        "template:exportMultiple",
        respond(async (request: MultipleTemplateExportRequest) => {
            const result = await dialog.showSaveDialog({
    ...
    ipcMain.handle(
        "template:clone",
        respond((template_id: number, new_name: string) => {
    ...
    ipcMain.handle(
        "templateItem:create",
        respond((request: CreateTemplateItemRequest) => templateService.createTemplateItem(request)),
    );

    ipcMain.handle(
        "templateItem:update",
        respond((request: UpdateTemplateItemRequest) => {
    ...
    ipcMain.handle(
        "templateItem:delete",
        respond((item_id: number) => templateService.deleteTemplateItem(item_id), { successFromBoolean: true }),
    );

    ipcMain.handle(
        "templateItem:safeDelete",
        respond((item_id: number) => templateService.safeDeleteTemplateItem(item_id)),
    );

    // Project handlers
    ipcMain.handle(
        "project:create",
        respond((request: CreateProjectRequest) => projectService.createProject(request)),
    );

    ipcMain.handle(
        "project:list",
        respond((filter?: { search?: string; status?: string; template_id?: number }) =>
    ...
    ipcMain.handle(
        "project:update",
        respond((request: UpdateProjectRequest) => {
    ...
    ipcMain.handle(
        "project:delete",
        respond((project_id: number) => projectService.deleteProject(project_id), { successFromBoolean: true }),
    );

    ipcMain.handle(
        "project:checkComplete",
        respond((project_id: number) => projectService.checkProjectComplete(project_id)),
    );

    // Attachment handlers
    ipcMain.handle(
        "attachment:upload",
        respond(async (project_item_id: number) => {
            // Show file picker
            const result = await dialog.showOpenDialog({
    ...
    ipcMain.handle(
        "attachment:list",
        respond((project_item_id: number) => attachmentService.listAttachments(project_item_id)),
    );

    ipcMain.handle(
        "attachment:delete",
        respond((attachment_id: number) => attachmentService.deleteAttachment(attachment_id), {
            successFromBoolean: true,
        }),
    );
    ...
    ipcMain.handle(
        "attachment:rename",
        respond((attachment_id: number, new_name: string) => {
    ...
    ipcMain.handle(
        "attachment:openExternal",
        respond(
            async (attachment_id: number, use_watermarked?: boolean) => {
                const path = attachmentService.getAttachmentFilePath(attachment_id, !!use_watermarked);
    ...
    ipcMain.handle(
        "watermark:apply",
        respond((attachment_id: number, req?: { watermark_text?: string; config?: WatermarkConfig }) =>
            watermarkService.applyWatermark(attachment_id, req?.watermark_text, req?.config),
        ),
    );
    ipcMain.handle(
        "watermark:delete",
        respond((attachment_id: number) => attachmentService.clearWatermark(attachment_id), {
            successFromBoolean: true,
        }),
    );
    ipcMain.handle(
        "watermark:resolveText",
        respond((attachment_id: number) => watermarkService.resolveWatermarkText(attachment_id)),
    );

    // Export/Import handlers
    ipcMain.handle(
        "project:export",
        respond(async (project_id: number) => {
    ...
    ipcMain.handle(
        "project:print",
        respond((project_id: number) => printService.printProject(project_id)),
    );
    // Confirm print: load the generated PDF and open system print dialog
    ipcMain.handle(
        "project:printConfirm",
        respond(
    ...
    // System helpers
    ipcMain.handle(
        "system:resolveStoragePath",
        respond(async (relative: string) => {
    ...
    ipcMain.handle(
        "document:create",
        respond((req: import("@common/types").CreateDocumentTemplateRequest) => documentService.createTemplate(req)),
    );
    ipcMain.handle(
        "document:list",
        respond((filter?: { search?: string }) => documentService.listTemplates(filter)),
    );
    ipcMain.handle(
        "document:get",
        respond((document_id: number) => {
    ...
    ipcMain.handle(
        "document:update",
        respond((req: import("@common/types").UpdateDocumentTemplateRequest) => {
    ...
    ipcMain.handle(
        "document:delete",
        respond((document_id: number) => documentService.deleteTemplate(document_id), { successFromBoolean: true }),
    );

    ipcMain.handle(
        "document:export",
        respond(async (request: import("@common/types").DocumentExportRequest) => {
    ...
    ipcMain.handle(
        ...
        respond((req: import("@common/types").CreateProjectDocumentRequest) =>
            documentService.createProjectDocument(req),
        ),
    );
    ipcMain.handle(
        "projectDocument:list",
        respond((project_id: number) => documentService.listProjectDocuments(project_id)),
    );
    ipcMain.handle(
        "projectDocument:get",
        respond((project_document_id: number) => {
    ...
    ipcMain.handle(
        ...
        respond((req: import("@common/types").UpdateProjectDocumentRequest) => {
    ...
    ipcMain.handle(
        ...
        respond((project_document_id: number) => documentService.deleteProjectDocument(project_document_id), {
            successFromBoolean: true,
        }),
    );
    ipcMain.handle(
        "projectDocument:exportPdf",
        respond(async (project_document_id: number) => {
            const d = documentService.getProjectDocument(project_document_id);
            if (!d) throw new Error("Project document not found");
